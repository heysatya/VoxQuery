...
* LLM-based SQL generation (Claude claude-sonnet-4-20250514) with sqlglot validation *(Note: Superseded by ADR-001, canonical model is Claude Haiku for latency/cost efficiency)*
...
* SQL is generated by Claude claude-sonnet-4-20250514 using retrieved schema context and conversation history *(Note: Superseded by ADR-001, using Claude Haiku)*
...
| Backend | Claude 3.5 Haiku (`claude-3-5-haiku-20241022`) | SQL generation, clarification, summary generation |
...
| LLM | Claude claude-sonnet-4-20250514 (Anthropic) *(Superseded by ADR-001: Haiku)* | Best-in-class SQL generation & schema reasoning |
...
4. Claude claude-sonnet-4-20250514 *(Superseded by ADR-001: Haiku)* generates SQL with schema context \+ conversation history injected
