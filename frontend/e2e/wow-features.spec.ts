...
test.describe('VoxQuery WOW Features E2E Automated Tests', () => {
  test('exercises Morning Briefing, Query Dock, Multi-Widget Workspace, Memory Graph, and Data Glass Panel', async ({ page }) => {
    ...
    const starterBtn = page.getByRole('button', { name: 'How did revenue perform last quarter?' });
    ...
    await page.screenshot({ path: 'e2e-screenshots/01-insight-view.png', fullPage: true });
    ...
    if (await pinBtn.isVisible()) {
      await pinBtn.click();
      await page.screenshot({ path: 'e2e-screenshots/02-workspace-pinned.png', fullPage: true });
    }
    ...
    if (await tableToggle.isVisible()) {
      ...
      await page.screenshot({ path: 'e2e-screenshots/03-table-view.png', fullPage: true });
    }
    ...
    if (await memGraphBtn.isVisible()) {
      ...
      await page.screenshot({ path: 'e2e-screenshots/04-memory-graph-modal.png', fullPage: true });
    }
  });
});
