...
async def main():
    ...
    if not db_url or not dsn or not fernet_key:
        print("Missing required environment variables (SUPABASE_DATABASE_URL, SNOWFLAKE_DSN, FERNET_KEY)")
        return
    ...
    try:
        ...
        for tenant in tenants:
            ...
            has_conn = await conn.fetchrow("SELECT 1 FROM tenant_connections WHERE tenant_id = $1", tenant_id)
            if not has_conn:
                await conn.execute("""
