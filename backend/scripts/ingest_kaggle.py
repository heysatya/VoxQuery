...
def process_path(input_path: str):
    ...
    try:
        if input_path.endswith('.zip'):
            ...
            print("Unsupported file format. Please provide a .zip file, a directory containing .csv files, or a single .csv file.")
            
    finally:
        conn.close()

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Ingest Kaggle datasets (ZIP, Folder, or CSV) into Snowflake")
    parser.add_argument("path", help="Path to the Kaggle .zip file, folder of CSVs, or single .csv file")
