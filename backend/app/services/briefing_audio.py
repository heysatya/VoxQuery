...
async def get_or_generate_briefing_audio_bytes(
    ...
    Checks Redis cache for key briefing_audio:{tenant_id}:{date}:{hash}.
    ...
    text_to_speak = f"{briefing.greeting}. {briefing.summary_narrative}"

    hash_key = hashlib.sha256(text_to_speak.encode("utf-8")).hexdigest()[:12]
    cache_key = f"briefing_audio:{claims.tenant_id}:{briefing.date}:{hash_key}"
