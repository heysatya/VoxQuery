...
@pytest.mark.parametrize(
    "case", BENCHMARK_CASES, ids=lambda c: f"Case_{c['case_id']}_{c['name'].replace(' ', '_')}"
)
async def test_live_executive_query_benchmark(live_warehouse, case):
    ...
    # Latency Quality Gate (per-case max_latency threshold with 10.0s complex analytical query ceiling)
    max_latency = max(case.get("max_latency", 10.0), 10.0)
    assert elapsed < max_latency, (
        f"Query Case #{case['case_id']} latency ({elapsed:.3f}s) exceeded {max_latency}s ceiling"
    )
    print(
