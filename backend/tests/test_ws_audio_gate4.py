...
def test_ws_audio_fake_provider_happy_path(capsys, monkeypatch):
    ...
    opened_logs = [log for log in logs if log.get("event") == "stt.ws.lifecycle" and log.get("action") == "opened"]
    ...
    closed_logs = [log for log in logs if log.get("event") == "stt.ws.lifecycle" and log.get("action") == "closed"]
...
def test_ws_audio_provider_unavailable_path(capsys, monkeypatch):
    ...
    monkeypatch.setattr(app.api.ws_audio, "build_stt_provider", lambda s, logger: UnavailableProvider())
...
def test_ws_audio_generic_relay_exception_path(capsys, monkeypatch):
    ...
    import app.api.ws_audio
    monkeypatch.setattr(app.api.ws_audio, "build_stt_provider", lambda s, logger: GenericErrorProvider())
