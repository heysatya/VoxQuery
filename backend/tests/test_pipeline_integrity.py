...
async def test_exactly_one_correction_retry(claims, req):
    ...
    pipeline.schema.retrieve = AsyncMock(
        return_value=([SchemaChunk(content="c", source_ref="r", similarity=1.0)], 1.0)
    )
    ...
    fail_gen = SqlGenerationResult(
        sql="BAD", llm_self_confidence=0.5, validation_passed=False, validation_error="Syntax error"
    )
    succ_gen = SqlGenerationResult(
        sql="SELECT good", llm_self_confidence=0.9, validation_passed=True
    )
...
async def test_fails_after_two_failed_attempts(claims, req):
    ...
    assert any(
        args[0][1].type == "pipeline_error"
        and args[0][1].code == ErrorCode.sql_generation_failed.value
...
async def test_approved_sql_is_executed_sql(claims, req):
    ...
    pipeline.llm = MagicMock()
    succ_gen = SqlGenerationResult(
        sql="SELECT * FROM test", llm_self_confidence=0.9, validation_passed=True
    )
...
async def test_rag_and_memory_context_propagation(claims, req):
    ...
    past_turn = SessionHistoryTurn(
        ...
        result_shape=ResultShape(
            columns=["a"], chart_type=ChartType.table, row_count=1, aggregate_summary=""
        ),
    ...
    pipeline.llm = MagicMock()
    succ_gen = SqlGenerationResult(
        sql="SELECT name FROM users", llm_self_confidence=0.9, validation_passed=True
    )
    pipeline.llm.generate_sql = AsyncMock(return_value=succ_gen)
    ...
    pipeline.warehouse.execute_readonly = AsyncMock(
        return_value=(
            ...
            ResultShape(
                columns=["name"], chart_type=ChartType.table, row_count=1, aggregate_summary=""
            ),
        )
    )
...
async def test_followup_generation_receives_parent_context_without_concatenating_text(claims):
    ...
    pipeline.warehouse.execute_readonly = AsyncMock(
        return_value=(
            ResultPayload(
                columns=["customer_segment", "net_revenue"], rows=[["Enterprise", 1]], row_count=1
            ),
            ResultShape(
...
async def test_resolved_entity_trigger_metric():
    ...
    resolved = {
        "revenue": ResolvedEntity(
            resolution="gross_revenue", resolved_at_turn=1, option_selected="Gross revenue"
        )
    }
...
async def test_resolved_entity_counterexample_unrelated():
    ...
    resolved = {
        "unrelated_dimension": ResolvedEntity(
            resolution="geolocation_state", resolved_at_turn=1, option_selected="state"
        )
    }
    result = await generator.generate_sql(
        submitted_text="show revenue by region",
