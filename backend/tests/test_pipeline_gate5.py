...
async def test_pipeline_enqueues_turn_with_identity():
    ...
    generation = SqlGenerationResult(
        sql="SELECT 1", llm_self_confidence=0.9, validation_passed=True
    )
    pipeline.llm.generate_sql = AsyncMock(return_value=generation)

    with (
        patch("app.services.graph.detect_ambiguity") as mock_detect,
        patch("app.services.graph.compute_confidence") as mock_conf,
    ):
        mock_detect.return_value = MagicMock(
            dominant_signal=None, signals_detected=[], ambiguous_terms=[]
        )
        mock_conf.return_value = MagicMock(
            clarification_triggered=False, confidence_tier="High", composite_score=0.9
        )
...
async def test_pipeline_resolves_ambiguity_with_actual_term():
    ...
    pipeline.llm = MagicMock()
    generation = SqlGenerationResult(
        sql="SELECT 1", llm_self_confidence=0.5, validation_passed=True
    )
    pipeline.llm.generate_sql = AsyncMock(return_value=generation)
    ...
    with (
        ...
        mock_detect.return_value = MagicMock(
            dominant_signal="entity_ambiguity",
            signals_detected=["entity_ambiguity"],
            ambiguous_terms=["customer_segment"],
        )
        mock_conf.return_value = MagicMock(
            clarification_triggered=True, confidence_tier="Low", composite_score=0.4
        )
